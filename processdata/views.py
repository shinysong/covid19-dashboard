...
def datamap_test(request):
    df = getdata.datamap_test()
    return HttpResponse(
        df.to_json(orient="records", force_ascii=False), content_type="application/json"
    )
...
def mapspage(request):
    ...
    return render(
        request, template_name="pages/maps.html", context={"usa_map": plot_div}
    )
