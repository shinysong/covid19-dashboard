...
def datamap_test():
    endpoint = "http://hike.cau.ac.kr/bigdata/namespace/datamap-test/sparql"
    ...
    SELECT distinct ?datamapName ?maintainer ?datasetCount ?organizationCount
    {
        ?datamap a dm:Datamap ;
                dm:datamapStat ?dmstat ;
                schema:maintainer/rdfs:label ?maintainer ;
                rdfs:label ?datamapName ;
                
                dm:dataset ?dataset . 
        ?dmstat dm:numberOfDataset ?datasetCount ;
                dm:numberOfOrganization ?organizationCount .
    }
    """
...
def usa_counties():
    ...
    populations = pd.read_csv(
        "https://raw.githubusercontent.com/balsama/us_counties_data/master/data/counties.csv"
